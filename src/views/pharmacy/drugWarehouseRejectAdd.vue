    ...
    <div class="left">
      ...
      <main class="grid dtc-list-grid mt-3">
        <DtxInputGroup prepend="退庫日期" labelWidth="120">
          <Calendar class="h-10 w-full" v-model="his.tiDrgApplyDate" placeholder="請輸入退庫日期" :showIcon="true" dateFormat="yy-mm-dd" />
        </DtxInputGroup>
        <DtxInputGroup prepend="退庫單號" labelWidth="120">
          <el-input v-model="his.chDrgApplyId" placeholder="請輸入退庫單號" />
        </DtxInputGroup>
        <DtxInputGroup prepend="退庫人員" labelWidth="120">
          <el-input v-model="his.chDrgApplyPersonName" placeholder="請輸入退庫人員" />
        </DtxInputGroup>
        ...
        <DtxInputGroup prepend="退庫藥房" labelWidth="120">
          <el-input v-model="his.chDrgApplyStoreName" placeholder="請輸入退庫藥房" />
        </DtxInputGroup>
      ...
      <div style="flex: 1" class="rounded-md overflow-y-auto grid my-3-grid px-4 mb-10" v-if="items.length">
        <nav v-for="(item, i) in items" :key="i" class="grid my-car-grid list-none" :class="!i ? 'mt-4' : 'mt-2'">
          <header style="grid-column: 1/-1" class="bg-blueGray-900 relative text-blueGray-100 text-left px-2 py-2 text-lg grid rounded-sm my-header">
            <div>退庫日期: {{}}</div>
            <div class="transform translate-x-7">退庫單號: {{ item.chDrgApplyId }}</div>
            <div>退庫人員:{{ item.chDrgApplyPersonName }}</div>
          ...
          <li>退庫藥房: {{ item.chDrgApplyPersonName }}</li>
